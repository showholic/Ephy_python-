# -*- coding: utf-8 -*-
"""
Created on Wed Jun 19 12:15:47 2019
...
def data_prep(pathname=r'C:\Users\Qixin\XuChunLab\nexdata\BaiTao\191545\06052019'):
...
#%% Raster
def plot_aux_raster(unit,trial_df,event_name,ax,prewindow=0.5,postwindow=0.5,aux_event=[],aux_id=[],
                linesize=0.5,shade_invalid=True,drop_invalid=True,plot_type=2):
    aux_trialind=get_aux_ind(trial_df,aux_event=aux_event,aux_id=aux_id)
    if drop_invalid:
        valid_index=np.argwhere(~np.isnan(trial_df['go cue'])).ravel()
        spikes=np.asarray(unit['spkt'])[valid_index]
        event=trial_df[event_name][valid_index]
    else:
        spikes=unit['spkt']    
        event=trial_df[event_name]
        
    if aux_trialind:    
        evt1=trial_df[event_name][aux_trialind[0]]
        evt2=trial_df[event_name][aux_trialind[1]]
        spk1=np.asarray(unit['spkt'])[aux_trialind[0]]
        spk2=np.asarray(unit['spkt'])[aux_trialind[1]]
        ta=1
        tb=-1
        for t,(spkt,evt) in enumerate(zip(spk1,evt1)):
            if ~np.isnan(evt): #no event timestamp in this trial -- no idea where to align
                if np.any(spkt): #make sure there is spike in the timewindow
                    ...
                    if np.any(spkevent):   
                        if aux_event:                           
                            ax.eventplot(spkevent,lineoffsets=ta,color='red',linelengths=linesize)
            ta+=1
        
        for t,(spkt,evt) in enumerate(zip(spk2,evt2)):
            if ~np.isnan(evt): #no event timestamp in this trial -- no idea where to align
                if np.any(spkt): #make sure there is spike in the timewindow
                    spktemp=spkt-evt
                    spkevent=slice_ts3(spktemp,-prewindow,postwindow)
                    if np.any(spkevent):   
                        if aux_event:                           
                            ax.eventplot(spkevent,lineoffsets=tb,color='green',linelengths=linesize)
            tb-=1
...
def plot_raster(unit,marker_df,event_name,ax,prewindow=0.5,postwindow=0.5,aux_event=[],
                linecolor=[0,0,0],linesize=0.5,shade_invalid=True,drop_invalid=True,plot_type=2):
...
def plot_raster_all_unit(Units,trial_df,event2align,prewindow=0.5,postwindow=0.5,aux_event=[],ncol=4,shade_invalid=True,drop_invalid=True,plot_type=2):   
    nrow=int(np.ceil(len(Units)/ncol))
    fig,ax=plt.subplots(nrow,ncol,sharex=True)  
    for n,unit in enumerate(Units):
        plot_raster(unit,trial_df,event2align,
                    ax[int(np.floor(n/ncol)),int((n)%ncol)],prewindow,postwindow,aux_event=aux_event,shade_invalid=shade_invalid,drop_invalid=drop_invalid,plot_type=plot_type)
    if aux_event:
